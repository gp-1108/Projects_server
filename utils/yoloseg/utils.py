...
def draw_detections(image, boxes, scores, class_ids, mask_alpha=0.3, mask_maps=None):
    ...
    # Generating one random color per each instance
    colors = rng.uniform(0, 255, size=(len(boxes), 3))

    mask_img = draw_masks(image, boxes, class_ids, mask_alpha, mask_maps, colors)
...
def draw_masks(image, boxes, class_ids, mask_alpha=0.3, mask_maps=None, colors=None):
    mask_img = image.copy()
